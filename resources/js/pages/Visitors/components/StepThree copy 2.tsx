...
export default function StepThree({ onPrev, onNext, initialData }: StepThreeProps) {
    ...
    const startCamera = async () => {
        ...
        detectInterval.current = setInterval(async () => {
            ...
            if (detections.length === 0) setFaceMessage("Aucun visage détecté");
            else if (detections.length > 1) setFaceMessage("Plusieurs visages détectés");
            else setFaceMessage("Visage détecté");
        }, 500);
    };
    ...
    const handleFinish = () => {
        ...
        if (!canvasSignRef.current) return alert("Veuillez signer");

        const signatureFile = dataURLtoFile(
            canvasSignRef.current.toDataURL(),
            "signature.png"
        );
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="rounded-lg border p-4 space-y-3">
                ...
                {selfiePreview && (
                    <div className="text-center space-y-2">
                        <img src={selfiePreview} className="w-40 h-40 mx-auto rounded-full border object-cover" />
                        <Button variant="outline" onClick={() => setSelfieFile(null)}>Reprendre</Button>
                    </div>
                )}
            </div>
            ...
            <div className="border rounded-lg p-4 space-y-3">
                ...
                <Input placeholder="Nom" value={emergencyName} onChange={e => setEmergencyName(e.target.value)} />
            ...
            <div className="flex justify-between">
                ...
                <Button onClick={handleFinish}>Suivant</Button>
            </div>
            ...
            <Dialog open={isModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Placez votre visage dans le cercle</DialogDescription>
                    </DialogHeader>

                    <div className="relative">
                        <video ref={videoRef} autoPlay muted className="w-full rounded" />
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="w-56 h-56 rounded-full border-4 border-primary border-dashed" />
                        </div>
                    </div>

                    <p className="text-center text-sm">{faceMessage}</p>

                    <DialogFooter>
                        <Button variant="outline" onClick={stopCamera}>Annuler</Button>
                        <Button onClick={captureSelfie} disabled={!faceMessage.includes("Visage")}>
                            Capturer
                        </Button>
                    </DialogFooter>
                </DialogContent>
